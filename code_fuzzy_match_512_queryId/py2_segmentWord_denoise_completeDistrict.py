...
def get_briefDistrictName():#1 level district
    districtNameBrief={}
...
if __name__=="__main__":
    ...
    fpath='../data_queryId/'+fname+'_segmented_deepcut.csv'
    ...
    for ind in range(segSerial.shape[0])[:]:
        string=segSerial[ind]
        query_idi=queryIdSerial[ind]
        string_deep=deepSegSerial[ind]
        # seg_string
        #print 'raw',string
        #eachChar(string)
        ...
        strList=complementDistrict(strList,briefNameDic) #->list -> 1level district
        strList=remove_repeat(strList);#print 'unique',' '.join(strI)  ->list

        if len(strList)>0:
            segSerial_denoise.append(' '.join(strList) )
            queryId_denoise.append(query_idi)
            raw_denoise.append(rawSerial[ind])
            ###
            string=remove_noise_notsplit(string_deep)
            strList=[st.strip(' ') for st in string.split(' ') if len(st.strip(' '))>=1];#print '2',strList#['abc','er'..]
            strList=remove_repeat(strList)
            strList=remove_digitString(strList);#print 'remove digit',' '.join(strI)  ->list
            strList=remove_noise(strList);#print 'remove noise',' '.join(strI)  ->list
            strList=complementDistrict(strList,briefNameDic) #->list -> 1level district
            strList=remove_repeat(strList)
            deepSegSerial_denoise.append(' '.join(strList) )
    ...
    pd.DataFrame({fname+'_seg':segSerial_denoise,fname+'_raw':raw_denoise,fname+'_deepSeg':deepSegSerial_denoise,'id':queryId_denoise}).\
        to_csv('../data_queryId/'+fname+'_segmentDenoise_deep.csv',index=False,encoding='utf-8')
