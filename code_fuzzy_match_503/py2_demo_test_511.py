...
def latlng2distance(Lat_A, Lng_A, Lat_B, Lng_B):#32 118
    ...
    distance = ra * (xx + dr)
    return distance
def getGeoDistance(test,candidate): #[1,2],[1,2]
    eps=0.0001
...
def filter_conflict_district(testDistrictList,candiArr,candiArr_deep,structureDistrictDic):#['beijing','haidian']
    ...
    for ij in range(len(candiArr[:])):#not deep seg string
        candStr=candiArr[ij]
        cand_districtList=structureDistrictDic[candStr]#[beijing,haidian]
        #print candStr,'|',cand_districtList[0],cand_districtList[1]
        if cand_districtList[0] in [testDistrictList[0],0]:#not conflict district level1
            if cand_districtList[1] in [testDistrictList[1],0]:#not conflict district level2

                ### strip level1 level2  '北京市' '海淀区' only level1 district is completement 市
                #print cand_districtList[1],cand_districtList[0]
                #print cand_districtList[1][:-1],cand_districtList[0][:-1]
                nameList=[cand_districtList[1],cand_districtList[0] ]#,cand_districtList[1][:-1],cand_districtList[0][:-1]]
                for name in nameList: #[0,0] [beijingshi,haidianqu] [beijingshi,0]
                    if name!=0:
                        candStr=candStr.replace(name,'')#not deep seg string
                ##  ??北京市 海淀 南路  ??北京市 海淀 cannot strip 'haidian'
                #
                #print candStr
                ####
                if len(candStr)>=2 and candStr.isdigit()==False:
                    candiListFiltered_deep.append(candiArr_deep[ij]);#print 'pass...'
                    candiListFiltered.append(candiArr[ij])
...
def tryMore_locatebyAddr(testStr):#seg str not deep
    #print 'to be found',testStr
    testList=testStr.split(' ')#list
    ####try 1 st time
    furtherTestStr=''.join(testList);#print 'try 1st',furtherTestStr
    locDic=locatebyAddr(furtherTestStr)
    if isinstance(locDic,dict):
        return locDic
    ############ try  more
    else:
        num=len(testList);midLen=int(num/2.)
        for i in range(testList.__len__())[::-1][:midLen]:# [5,4,3,2,1,0] ->[5,4,3]
            furtherTestStr=''.join(testList[:i]);#print 'try further',furtherTestStr
            locDic=locatebyAddr(furtherTestStr)
            if isinstance(locDic,dict):break
        ###
        if isinstance(locDic,dict):
            return locDic
        else:return None
...
def getKiloMeter(testStr,candInd,candiArr): #seg_str not deep
    noGeoDist=10000
    totGeoInfoList=[]

    #testLoc=locatebyAddr(testStr.replace(' ',''))#seg_string->noSeg_string
    testLocDic=tryMore_locatebyAddr(testStr)#dict or None
    #testLoc=[testLoc.values() if isinstance(testLoc,dict) else [0,0]][0];#print testLoc
    ########### test location not exist ??
    if isinstance(testLocDic,dict)==False:
        #testLoc=tryMoreLocation(testStr)#seg str not deep ->none or [jing ,wei ]
        #testLoc=[0,0]
        print 'no loc for test address'
        #totGeoInfoList.append('test no geo')
        totGeoInfoList=[ [candiArr[cind],noGeoDist] for cind in candInd]
    ####################3#test address location exist
    else:#test address location exist
        testLoc=testLocDic.values()
        candDistList=[]
        for ind in candInd:#after tfidf ind ,each candidate
            locCand=[0,0]
            #print candiArr[ind]
            #candStr_noSeg=candiArr[ind].replace(' ','')#'深圳市 宝安区'->'深圳市宝安区'
            locDic=tryMore_locatebyAddr(candiArr[ind])#seg str not deep->dict or none
            if isinstance(locDic,dict): #candidate str location exist
                locCand=locDic.values()
...
def score_editDist(testStr,testStr_deep,candList_deep,candList):#segDeepstr  :count how many words are hit
    print ' included string...'
    simCandList=[]
    testL_d=testStr_deep.split(' ')
    testStr_deep_noSeg=testStr_deep.replace(' ','')

    for ij in range(len(candList_deep)):#segStr deep

        cand=candList_deep[ij]#seg str deep
        candStr_deep_noSeg=cand.replace(' ','')
        candStr_noSeg=candList[ij].replace(' ','')
        #print cand
        ...
        ###
        editScore1=sum([1 for s in candList_deep[ij].split(' ') if s in testStr.replace(' ','') ])/float(len(candList_deep[ij].split(' ')))
        editScore2=sum([1 for s in testStr_deep.split(' ') if s in candList[ij].replace(' ','') ])/float(len(testStr_deep.split(' ') ))
        ...
        if 1 in [editScore1,editScore2]:
            print candList[ij]
            simCandList.append(candList[ij])
...
if __name__=="__main__":
    ...
    rngList=random.sample( range(string_deepSeg.shape[0]),1)#472535#355126#random.choice(range(string_seg.shape[0])) #6
    testDict_km={};testDict_tfidf={}#;within1kmTestDict={};strEditSimDict={};
    testDict_str={}
    for rng in rngList:
        rng=451663#282368#336812#115386#142636#451663 #18632
        query=string[rng];print 'query no seg str rng',rng
        ...
        ### remove those 'beijingshi haidianqu'
        ...
        candList=score_editDist(query_preprocess,query_preprocess_deep,candiArr_deep[candInd],candiArr[candInd])#deepSeg,->[ [segDeep,score],[]...]
        #strCloseList=[ [candiArr_deep[candInd[i]],dist_str[i]] for i in range(len(candInd)) ]#first column [strDeepSeg,score]
        testDict_str[query_preprocess]=candList
